...
def gastos():
    ...
    gastos_mensuales=db.session.execute(text(open('./static/sql/inicio/gastos_cambio_mensual.sql','r',encoding='utf-8').read()), {'id_usuario': session['id_usuario']}).first()
    ...
    return render_template("main/tableros/gastos.html", **data,gastos_anuales=gastos_anuales,gastos_mensuales=gastos_mensuales,anios=anios)
...
def ingresos():
    ...
    return render_template("main/tableros/ingresos.html", **data,ingreso_anual=ingreso_anual,ingreso_mensual=ingreso_mensual,anios=anios)
