...
def limpiar_bandera_auditoria(session):
    session._audit_logged = False
    session._audit_created_logged = False
    
def registrar_logs_auditoria(session, flush_context, instances):
    if getattr(session, "_audit_logged", False):
        return  # Ya se ejecutó
    session._audit_logged = True
    """Registrar logs de auditoría para las operaciones en la base de datos."""
    import json
    from datetime import date, datetime
...
def registrar_logs_post_flush(session, flush_context):
    if getattr(session, "_audit_created_logged", False):
        return 
    session._audit_created_logged = True
    """Registrar logs de auditoría después de que se asignen los IDs."""
    import json
