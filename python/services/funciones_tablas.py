...
def edit_on_success(table_name, id_):
    handler = HANDLERS.get(table_name)
    if not handler:
        return  # or log: unknown table
    return handler(id_)

@on_success('negocios_apple_pay')
def eos_negocios_apple_pay(id_):
    record = db.session.get(NegociosApplePay, id_)
    if not record or record.id_categoria_de_gasto is None:
        return

    # bulk update (faster than looping)
    (Gastos.query
