with categories as (
  ...
  where id_usuario=:id_usuario or id_usuario=:id_usuario_conectado
),
all_gastos as (
  SELECT 
      usuarios.nombre,
...
borrow_gastos as (
  select concat('4Préstamos hechos por ',all_gastos.nombre) as variable,coalesce(sum(importe_mensual*(pagos_mensuales-meses_pagados)),0) as value
  from all_gastos
  left join categories
    on all_gastos.id_categoria_de_gasto=categories.id
  where categories.nombre='Préstamos'
  and meses_pagados<pagos_mensuales
    and meses_pagados>0
  group by all_gastos.nombre
),
general_gastos as (
  select concat('1',all_gastos.nombre) as variable,coalesce(sum(importe_mensual*(pagos_mensuales-meses_pagados)),0) as value
  from all_gastos
  left join categories
    on all_gastos.id_categoria_de_gasto=categories.id
  where categories.nombre!='Préstamos'
...
total as (
  select '2Gasto' as variable, coalesce(sum(value),0) as value from general_gastos
),
divided as (
  select '3Gasto dividido' as variable, coalesce(sum(value)/2,0) as value from general_gastos
),
debt as (
...
debt2 as (
  select concat('5Deuda total (',variable,')') as variable,gasto-value as debt 
  from debt
