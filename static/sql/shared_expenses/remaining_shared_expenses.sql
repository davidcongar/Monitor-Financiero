...
borrow_gastos as (
  select concat('4Préstamos hechos por ',all_gastos.name) as variable,coalesce(sum(monthly_importe*(monthly_payments-payed_months)),0) as value
  from all_gastos
...
general_gastos as (
  select concat('1',all_gastos.name) as variable,coalesce(sum(monthly_importe*(monthly_payments-payed_months)),0) as value
  from all_gastos
  left join categories
    on all_gastos.id_categoria_de_gasto=categories.id
  where categories.name!='Préstamos'
